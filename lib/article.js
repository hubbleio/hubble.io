...
function Article(conf, repo, githubInfo) {
  ...
  function retrieveIntro() {
    ...
    if (! ret.meta || ! ret.markup) return;
    ret.meta.intro = ret.meta.description ? '<p>' + ret.meta.description + '</p>' : '';
    
    if (ret.meta.intro.length < INTRO_CHAR_COUNT) {
      // add para by para

       rest = ret.markup
         .trim()
         .replace('\n', '')
         .replace(/\<h(1|2|3)\>(?:(?!\<\/h(1|2|3)\>).)+\<\/h(1|2|3)\>/mg, '')
         .replace(/\<iframe.*\<\/iframe\>/g, '')
         ;

       if(ret.meta.intro.length < INTRO_CHAR_COUNT && rest) {
         nextP = rest.indexOf('</p>');
         if (nextP >= 0) {
          ret.meta.intro += '<div class="rest">';
          ret.meta.intro += rest.slice(0, nextP + 4);
          rest = rest.slice(nextP + 4);
          ret.meta.intro += '<p>...</p>';
          ret.meta.intro += '</div>';
         }
       }
